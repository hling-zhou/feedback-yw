...
describe('FeedbackDrawer layout', () => {
  ...
  it('places anchor nav in drawer title and orders content sections', () => {
    ...
    const handling = marker('page-card-title">处理意见（工单原文）</span>')
    const rootCause = marker('page-card-title">根因排查</span>')
    const followUp = marker('page-card-title">回访满意度</span>')
    ...
    const opt = marker('page-card-title">优化建议</span>')
    const note = marker('page-card-title">备注</span>')
    ...
    const causeSwitch = marker('发起投诉原因复核')
    ...
    expect(causeFinal).toBeLessThan(causeSwitch)
    expect(causeSwitch).toBeLessThan(causeReview)
    expect(src).toContain('complaintCauseReviewEnabled')
    expect(src).toContain('isCompleteComplaintCauseReview')
    expect(src).toContain('closable={{ placement: \'end\' }}')
    expect(src).toContain('TICKET_DETAIL_DRAWER_WIDTH')
    expect(src).toContain('DeleteTicketConfirmModal')
    expect(src).toContain('canDeleteTicket')
  ...
  it('delegates close to parent and tracks dirty state for leave confirm', () => {
    ...
    expect(src).toContain('TICKET_TODO_MANUAL_RESOLUTION_SELECT_OPTIONS')
    expect(src).toContain('TICKET_TODO_RESOLUTION_SELECT_OPTIONS')
  })
})
