...
export const SETTINGS_TAB_DESCRIPTIONS = {
  ...
  analysis:
    '团队共享的自动打标与分析规则；仅管理员可改，修改后底部会出现保存条，保存后约 5 秒内同步给其他用户。',
  metrics:
  ...
  requirement_sync:
    '查看外部需求工单进展与状态映射及外部系统 API Key；仅管理员可编辑映射和创建/吊销 Key，举措关联需求工单后排期与状态由此同步展示。',
  knowledge_base:
    '上传/管理各产品的业务知识库，供工单自动分析的「优化建议」检索引用；同一产品上传即覆盖。',
  pipeline: '仅管理员可运行行动建议规则补充流水线：Producer 归因 → 新词发现 → 自动写入 → 门禁验证；跑完在页面查看结果。',
  curated_taxonomy:
    '查看各产品的行动建议分类法（curated JSON）；仅管理员可编辑 family 正则与子议题，保存后写入服务端，刷新洞察即生效。',
}
...
export function getVisibleSettingsTabs(can) {
  /** @type {SettingsTabKey[]} */
  const tabs = []
  if (can('view')) tabs.push('llm')
  if (can('view')) tabs.push('analysis')
  ...
  if (can('view')) tabs.push('requirement_sync')
  if (can('manageKnowledgeBase')) tabs.push('knowledge_base')
  if (can('view')) tabs.push('pipeline')
  if (can('view')) tabs.push('curated_taxonomy')
  return tabs
}
