...
export function topicFromUserQuery(query, options = {}) {
  ...
  return {
    ...
    problemKey: type === 'customer' ? undefined : (parsed?.problemText || parsed?.tokens.filter((token) => !parsed.productTokens.includes(token)).join('') || text),
    matchLayers: type === 'customer' ? undefined : parsed?.layers,
    whyNow: '用户指定专题',
    sourceHint: '用户新建',
    sampleSize: 0,
    sourceTypes: [],
