...
const COL = {
  ...
  enabled: ['是否启用', '启用', 'enabled', '投诉咨询分析', '工单分析'],
  analysisPostUseRating: ['用后即评分析', 'analysisPostUseRating', '用后即评'],
  focusTracked: ['重点跟踪', 'focusTracked', '用后即评重点跟踪'],
...
export function parseProductCatalogWorkbook(buffer) {
  ...
  for (const row of productRows) {
    ...
    byKey.set(key, {
      ...
      analysisPostUseRating: parseEnabled(pick(row, COL.analysisPostUseRating)),
      focusTracked: parseEnabled(pick(row, COL.focusTracked)),
