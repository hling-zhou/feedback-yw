...
export const TICKET_LLM_FILTER_HINTS = {
  ...
  needs_llm:
    '投诉/咨询工单中，客户请求内容或需求痛点至少有一项的来源不是「大模型」（含规则初标、人工编辑、导入回写）。工单 LLM 会一次写出客户请求、需求痛点、问题原因与优化建议。常见原因：导入时未配置 API Key、LLM 调用失败或尚未补打。用户旅程请用「待增强 · 用户旅程」筛选。',
  full_llm:
    '投诉/咨询工单中，客户请求内容与需求痛点的来源均为「大模型」。工单 LLM 会同时写入问题原因与优化建议；旅程是否完成增强请查看详情来源 Tag 或「待增强 · 用户旅程」筛选。',
  needs_journey_llm:
