...
export async function enrichRecordWithTicketLlm(record, settings) {
  ...
  const ruleFallback = extractTicketOptimizations({
  ...
  let optimizationProduct = record.optimizationProduct?.trim() || ruleFallback.optimizationProduct
  let optimizationService = record.optimizationService?.trim() || ruleFallback.optimizationService
