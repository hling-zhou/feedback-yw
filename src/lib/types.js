 ...
 * @property {string} [rootCauseReview] - 根因排查（人工复核；仅用户编辑保存后写入。导入列「问题原因」若为终判树不预填、不视为复核）
 ...
 * @property {{ requestScene?: string; problemType?: string; journeyL1?: string; journeyL2?: string; taggedAt?: string }} [lastAutoTags] - 最近一次系统打标快照，改标学习对照用
...
const ALL_SOURCES = ['complaint_ticket', 'consultation_ticket', 'post_use_rating', 'user_survey', 'other']
const TICKET_SOURCES = ['complaint_ticket', 'consultation_ticket']
const COMPLAINT_SOURCES = ['complaint_ticket']
const POST_USE_SOURCES = ['post_use_rating']
...
export const STANDARD_FIELDS = [
  {
    ...
    label: '工单流水号',
    required: false,
    dataSourceTypes: TICKET_SOURCES,
    hint: '导入时若表头含「工单展示流水号」或「工单流水号」将自动映射（前者优先）；也可手动选择其它列。',
  },
  { key: 'createdAt', label: '创建时间', required: false, dataSourceTypes: ALL_SOURCES },
  {
    key: 'productSpec',
    label: '投诉产品',
    required: false,
    dataSourceTypes: ALL_SOURCES,
    hint: '映射到工单 Excel 中的投诉产品列，常用列名「具体投诉产品」；系统据此匹配「目标产品」范围与规格。',
  },
  { key: 'resourcePool', label: '所属资源池', required: false, dataSourceTypes: TICKET_SOURCES },
  {
    key: 'customerTierCol',
    label: '客户等级',
    required: false,
  ...
  { key: 'customerTypeNameCol', label: '客户类型名称', required: false, dataSourceTypes: TICKET_SOURCES },
  { key: 'groupNameCol', label: '集团名称', required: false, dataSourceTypes: TICKET_SOURCES },
  { key: 'groupCustomerCodeCol', label: '集团客户编码', required: false, dataSourceTypes: TICKET_SOURCES },
  { key: 'groupProvinceCol', label: '集团所属省份', required: false, dataSourceTypes: TICKET_SOURCES },
  { key: 'groupCityCol', label: '集团所属地市', required: false, dataSourceTypes: TICKET_SOURCES },
  {
  ...
  { key: 'rawText', label: '受理内容 / 主文本', required: false, dataSourceTypes: ALL_SOURCES, hint: '投诉/咨询工单：客户侧问题描述，常用列「受理内容」；可与下方「合并到主文本」列拼接。非工单来源时映射正文列。' },
  { key: 'handlingText', label: '处理意见（打标必填）', required: true, dataSourceTypes: TICKET_SOURCES, hint: '客服处理记录，四维打标与客户请求/痛点抽取的主要语料；投诉工单常用列「处理意见」。' },
  { key: 'responseText', label: '优化举措/建议', required: false, dataSourceTypes: TICKET_SOURCES, hint: '可选。映射工单表「优化举措/建议」列，写入 solutionSummary，供工单详情展示与规则/LLM 优化建议参考；未映射时尝试从处理意见等文本解析。' },
  { key: 'rootCauseCol', label: '问题原因', required: false, dataSourceTypes: TICKET_SOURCES, hint: '可选。映射工单表「问题原因」列，写入 rootCause，用于规则优化建议与 LLM 上下文；≠ 投诉原因（终判）。未映射时尝试从正文解析。' },
  {
    key: 'problemTypeL1FinalCol',
    label: '投诉原因一级（终判）',
    required: false,
    dataSourceTypes: COMPLAINT_SOURCES,
    hint: '写入投诉原因（终判）字段与原始列快照；不参与「问题类型」自动打标。',
  },
  { key: 'problemTypeL2FinalCol', label: '投诉原因二级（终判）', required: false, dataSourceTypes: COMPLAINT_SOURCES },
  { key: 'problemTypeL3FinalCol', label: '投诉原因三级（终判）', required: false, dataSourceTypes: COMPLAINT_SOURCES },
  { key: 'source', label: '渠道', required: false, dataSourceTypes: ALL_SOURCES },
  { key: 'visitMonth', label: '回访月份', required: false, dataSourceTypes: POST_USE_SOURCES },
  { key: 'productName', label: '产品名称', required: false, dataSourceTypes: POST_USE_SOURCES },
  { key: 'customerName', label: '客户名称', required: false, dataSourceTypes: POST_USE_SOURCES },
  { key: 'customerCode', label: '客户编码', required: false, dataSourceTypes: POST_USE_SOURCES },
  { key: 'feedbackSummary', label: '客户反馈摘要', required: false, dataSourceTypes: POST_USE_SOURCES },
  { key: 'userFeedbackText', label: '用户反馈原文', required: false, dataSourceTypes: POST_USE_SOURCES },
  { key: 'scoreSource', label: '评分来源', required: false, dataSourceTypes: POST_USE_SOURCES },
  { key: 'ratingText', label: '用后即评评分', required: false, dataSourceTypes: POST_USE_SOURCES },
  { key: 'userInfo', label: '用户信息', required: false, dataSourceTypes: POST_USE_SOURCES },
  { key: 'userInfoDetail', label: '用户信息详情', required: false, dataSourceTypes: POST_USE_SOURCES },
  { key: 'visitResult', label: '回访结果', required: false, dataSourceTypes: POST_USE_SOURCES },
  { key: 'visitFeedbackDetail', label: '回访反馈信息', required: false, dataSourceTypes: POST_USE_SOURCES },
  { key: 'internalConclusion', label: '内部结论', required: false, dataSourceTypes: POST_USE_SOURCES },
  { key: 'internalEvaluationDetail', label: '回访反馈信息-内部评估', required: false, dataSourceTypes: POST_USE_SOURCES },
  { key: 'jiraId', label: '内部提单', required: false, dataSourceTypes: POST_USE_SOURCES },
]
