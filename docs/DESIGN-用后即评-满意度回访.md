...
> 设计总览：[DESIGN-20260601-1.md](./DESIGN-20260601-1.md)  
> 任务分解：[TASKS-用后即评-满意度回访.md](./TASKS-用后即评-满意度回访.md)  
> 状态：**已实现**（P0–P6，2026-06-05）

本文描述「用户即评」下 **满意度回访记录报表** 的导入、工单补全、展示、导入导出闭环，以及洞察工作台 **回访满意度** 分析模块。官网渠道问卷、短信渠道问卷后续单独设计。
...
系统已有 `dataSourceType: post_use_rating` 及 Stub Pipeline，用于 **独立评价记录**（`ratingScore` 等）。

**满意度回访** 的本期方案是：

- **不**将回访行导入为新的 `post_use_rating` 记录；
- 将回访信息 **补全（enrichment）** 到已存在的 **投诉 / 咨询工单** 上；
- 工作台「用后即评」Tab 内增加 **回访满意度** 模块，基于「有回访信息的工单子集」分析。
