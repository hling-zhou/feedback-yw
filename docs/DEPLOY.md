...
- [ ] 若本次发版含 **工单/举措编辑冲突检测**（`recordRevision`）：**重启 API** 后再验收（旧进程不会写入版本号）
- [ ] 两人同时打开同一条工单详情，先后保存 → 后保存者出现冲突弹窗；可选「加载最新 / 覆盖保存 / 取消」
- [ ] 两人同时编辑同一条举措 → 后保存者出现冲突弹窗（`ACTION_ITEM_CONFLICT`）
- [ ] 导入或批量打标进行中，工单详情「保存」按钮禁用并提示原因
...
- 表 `audit_log`：记录导入批次、清空数据、打标/产品目录发布、用户 CRUD、**工单冲突强制覆盖**（`storage.record_force_overwrite`）等
- 查询：`GET /api/audit?days=7`（需权限 `viewAudit`，**全部角色**具备；默认最近 7 天，最多 90 天）
- 管理界面：**设置**页底部「操作审计」卡片（标题旁刷新）
