...
1. 结论板：P0/P1/P2 色块、触发信号，以及做什么 / 谁来做 / 多急 / 怎么验证四格。
2. 指标条：匹配、负向、近期/基线、产品数、开放举措，附月度迷你柱。
3. 定性 / 归因图卡：一句判断、置信度、维度占比条及可打开详情的工单引用。
4. 原话卡片、举措状态行和补充材料芯片。

工单号在**当前报告页**打开详情抽屉，不跳转反馈库。旧报告没有 `decision` 时提示按系统数据重算。
...
`collectEvidence` 将匹配记录压成 `signalPack`：统一的近期/基线月桶、推荐规则场景、标签与维度集中度、举措状态、样本和弱预期词信号。`buildDecision` 再确定定性、P 级、归因假设与行动。

- P 级只由趋势 × 高价值 × 扩散查表确定；有举措不会自动降级。
- 匹配少于 5 条时归因强制低置信；建议拆专题时只能先拆，不能直接立项修复。
- LLM 只能润色定性、归因和行动的短文案，不能改 P 级、数字、状态、角色或验证方式。
- `sourceIds` 必须来自证据 id / 工单号 / 补充材料 id；不合法输出丢弃并回退规则决策。
...
- 可删除本人创建的报告；管理员可删除任何人的报告（含未知创建人）。无导出包、无分享链接；详情 URL 仅本系统登录用户可打开。
